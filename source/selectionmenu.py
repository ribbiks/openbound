...
class SelectionMenu:
	def __init__(self, mapfile_list, pos, font, sort_field=0):
		self.pos     = pos
		self.content = []
		self.font    = font
		self.index   = 0
	...
	def draw(self, screen):
		if not self.content:
			offset = Vector2(4,4)
			self.font.render(screen, 'no maps found...', self.pos + offset)
		else:
			...
			for i in range(self.current_range[0], self.current_range[1]):
				...
				self.font.render(screen, self.content[i][0], self.pos + offset)
